...
import { Mail, ArrowLeft, Loader2, Send, Activity } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast({
        type: 'success',
        title: 'Email Sent',
        message: 'If an account exists with this email, a reset token has been sent to your inbox.'
      });
      ...
      <div className="w-full max-w-[400px]">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-[14px] text-zinc-500 mt-2 text-center max-w-[300px]">
            Enter your email and we'll send you a short token to reset your password.
          </p>
        </div>
        ...
        <div className="bg-[#FDFDFD] border border-zinc-200 rounded-[20px] p-6 sm:p-8 shadow-sm">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  required
                />
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 w-[18px] h-[18px]" strokeWidth={1.5} />
              </div>
            </div>
            ...
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send strokeWidth={1.5} className="w-4 h-4" />}
